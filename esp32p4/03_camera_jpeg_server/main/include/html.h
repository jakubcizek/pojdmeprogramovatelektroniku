...
"setIdle();"
"bJ.onclick=()=>{ if(bJ.disabled) return; clr(); img=new Image(); img.src='/capture-jpeg?quality='+qval(); v.appendChild(img); };"
"bS.onclick=()=>{ if(bS.disabled) return; setMJPEG(); clr(); img=new Image();"
"  img.src = location.protocol+'//'+location.hostname+':81/?quality='+qval();"
"  v.appendChild(img);"
"};"
