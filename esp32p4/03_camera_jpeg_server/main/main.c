...
#include "wifi.h"            // Wi-Fi se stará o připojení k síti. Nastavte SSID/heslo ve wifi.h 
...
#include "fonts/roboto60.h"  // V komponentě GFX najdete podadresář fonts s rastrovými písmy, které můžete vytvářet ve skriptu fontgen.py
...
void app_main(void){
    ...
    // Toto je funkce z naší komponenty a v jejím hlavičkovém souboru musíte nastavit SSID a heslo
    // Přihlašovací údaje nejsou cíleně součástí uživatelského kódu
    wifi_connect();
...
static esp_err_t webhandler_mjpeg_loop(httpd_req_t *http_req){
    ...
    while (true) {
        ...
        gfx_draw_rect(&cc, 15,15, cc.width-30, cc.height-30, 0, 0, 0, 5);
        gfx_draw_rect(&cc, 20,20, cc.width-40, cc.height-40, 255, 255, 255, 5);
        int tw = gfx_get_text_width(&Roboto60, "ESP32-P4 snímek 00000");
        int x = (int)cc.width/2 - tw/2;
        char msg[50];
        sprintf(msg, "ESP32-P4 snímek %d", frame_counter);
        ...
        frame_counter++;
        // Počkáme 50 ms, čímž stream zpomalíme a pomůže i všem bufferům po cestě
