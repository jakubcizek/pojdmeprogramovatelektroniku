...
uint32_t stahni_a_dekomprimuj() {
  uint32_t stazeno = 0;
  HTTPClient http;
  http.setTimeout(10000); // Zvysimu timeout pro TCP spojeni na 10 sekund, pokud budeme bitmapu generovat pri HTTP pozadavku
  ...
  if (kod == HTTP_CODE_OK) { // Pokud jsme navazali HTTP spojeni (odpoved HTTP OK)
    ...
    WiFiClient* stream = http.getStreamPtr(); // Otevreme strem na stahovana data
    ...
    while (http.connected() && (stazeno < velikost_soubor)) { // Pokud jsme spojeni se serverem a jeste jsme nestahli cely soubor
      size_t k_dispozici = stream->available(); // Zjistime velikost aktualne stazenych dat v zasobniku
      if (k_dispozici) { // Pokud tam nejake jsou
        uint8_t buffer[128];
        size_t precteno = stream->readBytes(buffer, 128); // Precteme 128 bajtu do naseho docasneho zasobniku
        if (precteno) {
          // RLE dekomprese davky v bufferu
          ...
          for (size_t pozice = 0; pozice < precteno; pozice += 2) { // Precteme vzdy dva bajty z naseho zasobniku. Prvni bajt je pocet, druhy bajt hodnota
            ...
            for (uint8_t pozice = 0; pozice < pocet; pozice++) bitmap_buffer[dekomprimovane_bajty++] = hodnota; // Vypnime bitmapu davkou
          }
          stazeno += precteno; // Navysime pocitaldo stzenych bajtu
        }
      }
...
void setup() {
  ...
  SPI.begin();
  // Vytvoreni blok upameti pro bitmapu
  ...
  // Displej se tedy prekresluje jednou z ahodinu
