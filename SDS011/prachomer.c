...
{
    uint8_t _checksum = 0;
    for (uint8_t i = zacatek; i <= konec; i++)
        _checksum += data[i];
    return _checksum;
}
...
int8_t precti_koncentraci(int fd, float *pm25, float *pm10, const uint32_t limit_us, uint32_t *doba)
{
    ...
    while (_doba <= _limit)
    {
        ...
        else if (ok == 1 && stav == 2 && pocitadlo == 2)
            stav = 3;
        else if (ok == 1 && stav == 3 && pocitadlo == 9 && bajt == 0xAB)
            stav = 4;
...
void uspat_senzor(int fd)
{
    ...
    {
        if (vypisovat_podrobnosti)
            printf("Odesilam bajt %02X ... ", spanek[i]);
...
void probudit_senzor(int fd)
{
    ...
    {
        if (vypisovat_podrobnosti)
            printf("Odesilam bajt %02X ... ", budicek[i]);
...
int main(int argc, char *argv[])
{
    ...
    for (uint8_t i = 3; i < argc; i++)
    {
        if (strcmp(argv[i], "-c") == 0)
        {
            ...
            if (vypisovat_podrobnosti)
                printf("Nastavuji pocet mereni na %d\r\n", opakovani);
            i++;
        }
        ...
        if (strcmp(argv[i], "-t") == 0)
        {
            ...
            if (vypisovat_podrobnosti)
                printf("Nastavuji max. dobu cekani na vysledek na %lu us\r\n", limit_us);
            i++;
        }
    ...
    for (uint8_t i = 0; i < opakovani; i++)
    {
        ...
        else
        {
            ...
            {
            case -2:
                printf("Chybna detekce, hodnoty jsou abnormalne vysoke!\r\n");
                break;
            case -1:
                printf("Prekrocen casovy limit!\r\n");
                break;
            case 0:
                printf("Chybna seriova data, checksum neodpovida!\r\n");
                break;
            default:
